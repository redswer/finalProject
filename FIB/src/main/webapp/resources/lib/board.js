...
function toggleAnswerSort(pageNumber) {
	...
	let showOnlyAnswered = checkbox.checked ? 0 : 1;
	
	let url=`board/pageInquiryListAdmin?answer_check=${showOnlyAnswered}&page=${pageNumber}`;
	
	console.log("체크박스 상태(0:미답변, 1:답변완료) :", showOnlyAnswered);
	
	axios.get(url
    ).then(response => {
        document.getElementById('managementArea').innerHTML = response.data;
     /* 아래의 두 문장 중 주석처리된 방식은 제대로된 view를 보여주지 못했다.
     	=> 체크박스 클릭시 답변상태 값 0 이 전달되서 답변상태 값이 0인 데이터만 정렬됐다.
     	   하지만, 체크박스가 체크되지않고 바로 초기화된다랄까.. 
     	   그로인해 view상태가 어떻든 체크박스를 클릭하게되면 답변상태 값이 0인 데이터만 출력됐다.
     */
     /* document.getElementById("inquiry_answer_toggle").value = showOnlyAnswered; */
        document.getElementById("inquiry_answer_toggle").checked = checkbox.checked;
